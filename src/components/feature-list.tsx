...
export default function FeatureList() {
    return (
        <div className="flex gap-8 w-full justify-center items-center relative mt-20">
            {features.map((feature, id) => (
                <div key={id} className={`${id === 0 ? '-rotate-6' : id === 1 ? 'rotate-0' : 'rotate-6'} group hover:rotate-0 hover:z-10 transition-transform hover:scale-125 duration-500`}>
                    <div className="border rounded-lg shadow-sm hover:shadow-md transition-shadow h-full w-60">
                        <div className={`${feature.background} h-80 w-60 rounded-t-lg relative overflow-clip`}>
            ...
            <div className="absolute top-40 -left-10 -z-10 rotate-2 h-32 w-full bg-linear-to-r from-blue-300/70 to-transparent" />
            {/* <div className="absolute top-20 -right-10 -z-10 rotate-2 h-32 w-full bg-linear-to-l from-fuchsia-300/70 to-transparent" /> */}
            <div className="absolute top-60 -right-5 -z-20 rotate-2 h-32 w-full bg-linear-to-l from-emerald-300/70 to-transparent" />
        </div>
    )
}
    ...
    return (
        <div className="min-w-96 bg-white p-4 rounded-lg shadow-md text-sm grayscale group-hover:grayscale-0 duration-500">
            <div className="mb-4 relative">
                ...
                <h3 className="absolute top-0 left-0 text-lg font-semibold z-[100]">Request Schedule</h3>
                <p className="text-gray-400 pt-2">Input information for your schedule request below.</p>
            </div>
            ...
            <div>
                {eventRequestItem.slice(0, 3).map((item, id) => (
                    <div key={id} className="grid grid-cols-4 gap-8 py-2 border-b">
                        <span className="min-w-64 flex items-center gap-2 grayscale group-hover:grayscale-0 duration-500">
